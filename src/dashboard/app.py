...
with st.sidebar:
    ...
    limit = st.slider("Number of candles", min_value=10, max_value=200, value=50, step=10)
...
def fetch_predictions(symbol: str, limit: int):
    ...
    try:
        response = requests.get(
            f"{API_URL}/predict/logistic/{symbol}",
            params={"limit": limit},
            timeout=10
        )
...
with col2:
    ...
    st.metric("Confidence", f"{conf:.2f}%", delta=f"{conf - 50:.2f}% vs 50%", delta_color=color)
...
fig.add_trace(go.Scatter(
    x=df["timestamp_iso"], y=df["open"],
    name="Open",
    line=dict(color="#888780", width=1, dash="dot")
), row=1, col=1)

fig.add_trace(go.Scatter(
    x=df["timestamp_iso"], y=df["close"],
    name="Close",
...
fig.add_trace(go.Scatter(
    x=df["timestamp_iso"], y=df["confidence_pct"],
    name="Confidence",
    fill="tozeroy",

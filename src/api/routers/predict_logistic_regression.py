...
async def fetch_klines(symbol: str, limit: int) -> pd.DataFrame:
    ...
    try:
        ...
        ).sort("open_time_ms", -1).limit(fetch_limit)
...
def run_prediction(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["timestamp_iso"] = pd.to_datetime(df["open_time_ms"], unit="ms").dt.strftime("%Y-%m-%dT%H:%M:%SZ")
...
async def predict(
    ...
    if df.empty:
        raise HTTPException(
            status_code=404,
            detail=f"No data found for {symbol} {INTERVAL} in MongoDB."
        )
